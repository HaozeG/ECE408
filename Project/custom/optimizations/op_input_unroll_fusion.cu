...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int B, const int M, const int C, const int H, const int W, const int K,const int S)
{
    ...
    */
    ...
    int h_unroll, w_unroll, c, h, w;
    w_unroll = colomn % (H_out * W_out);
    ...
    for (int i = 0; i < (numAColumns - 1)/BLOCK_SIZE + 1; ++i) {
        ...
        if ((i * BLOCK_SIZE + threadIdx.y) >= numBRows || colomn >= numBColumns) {
            ...
            // out_3d(b, h_unroll, w_unroll) = in_4d(b, c, h, w);
            h_unroll = i * BLOCK_SIZE + threadIdx.y;
            c = h_unroll / (K * K) % C;
            h = w_unroll / W_out * S + h_unroll / K % K;
            w = w_unroll % W_out * S + h_unroll % K;
            subTileB[threadIdx.y][threadIdx.x] = in_4d(b, c, h, w);
        }
        __syncthreads();
